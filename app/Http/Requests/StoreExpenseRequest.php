...
class StoreExpenseRequest extends FormRequest
{
    ...
    {
        return Auth::check(); // Any authenticated user can store their own expense
    }
    ...
    protected function passedValidation()
    {
        ...
        if (empty($descriptions) && !$this->has('description_is_optional_marker_for_empty_allowed')) { // Allows to bypass if explicitly needed
            $validator = $this->getValidatorInstance();
            if ($validator->errors()->isEmpty()) { // Only add if no other errors for description
                 $validator->errors()->add('description', 'At least one description (English, German, or Arabic) must be provided.');
            }
        }
        
        // Convert is_business_expense to boolean if present
        if ($this->has('is_business_expense')) {
            $this->merge(['is_business_expense' => $this->boolean('is_business_expense')]);
        } else {
            $this->merge(['is_business_expense' => false]);
        }
    }
}
